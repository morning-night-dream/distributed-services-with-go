...
func (l *DistributedLog) setupRaft(dataDir string) error {
	...
	hasStore, err := raft.HasExistingState(
		l.raftLog,
		stableStore,
		snapshotStore,
	)
	...
	if l.config.Raft.Bootstrap && !hasStore {
...
// わざわざ単発のrecordを引数にとるStoreLogを定義する意図は？
...
func (s *StreamLayer) Accept() (net.Conn, error) {
	...
	if !bytes.Equal([]byte{byte(RaftRPC)}, b) {
		return nil, fmt.Errorf("note a raft rpc")
	}
