...
func TestServer(t *testing.T) {
	for scenario, fn := range map[string]func(
		t *testing.T,
		rootClient api.LogServiceClient,
...
func setupTest(t *testing.T, fn func(*Config)) (
	...
	if *debug {
		...
		telemetoryExporter, err = exporter.NewLogExporter(exporter.Options{
			MetricsLogFile:    metricsLogFile.Name(),
			TracesLogFile:     tracesLogFile.Name(),
			ReportingInterval: time.Second,
		})
		require.NoError(t, err)
		err = telemetoryExporter.Start()
		require.NoError(t, err)
	}
...
func testProduceConsume(t *testing.T, client, _ api.LogServiceClient, config *Config) {
...
func testConsumePastBoundary(t *testing.T, client, _ api.LogServiceClient, config *Config) {
	ctx := context.Background()
	...
	if got != want {
		t.Fatalf("go err: %v, want: %v", got, want)
	}
}

func testProduceConsumeStream(t *testing.T, client, _ api.LogServiceClient, config *Config) {
	ctx := context.Background()
